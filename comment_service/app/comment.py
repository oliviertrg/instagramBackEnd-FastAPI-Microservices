...
async def comment(post_id: str,comments : schema.comments,current_users : int = Depends(auth2.get_current_user)):
    try:      
...
async def delete_comment(comment_id: str,current_users : int = Depends(auth2.get_current_user)):
   try : 
